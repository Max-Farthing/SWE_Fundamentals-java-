...
public class WolfSchedulerTest {
	 ...
	 * @throws Exception if IO path is not valid
	 */
	@Before
	...
	/** 
	 * Tests for conflict in Course events
	 */
	@Test
	public void testAddScheduleConflict() {
		WolfScheduler ws = new WolfScheduler(validTestFile);

		Activity a = new Course("CSC 116", "Intro to Programming - Java", "002", 3, "spbalik", "MW", 1120, 1310);
		//Activity b = new Course("CSC 230", "C and Software Tools", "001", 3, "dbsturgi", "MW", 1145, 1300);
		
		assertTrue(ws.addCourseToSchedule("CSC 116", "002"));
		assertEquals(1, ws.getScheduledActivities().length);
		assertEquals(1, ws.getFullScheduledActivities().length);
		String [] course = ws.getFullScheduledActivities()[0];
		assertEquals("CSC 116", course[0]);
		assertEquals("002", course[1]);
		assertEquals("Intro to Programming - Java", course[2]);
		assertEquals("" + CREDITS, course[3]);
		assertEquals("spbalik", course[4]);
		assertEquals(a.getMeetingString(), course[5]);
		assertEquals("", course[6]);
	...
	/**
	 * Test adding events with conflicting times
	 */
	@Test
	public void testAddEventConflict() {
		...
		try {
			ws.addEventToSchedule("Gym", "MW", 800, 900, "Weight lifting");
			fail();
		} catch(IllegalArgumentException e) {
